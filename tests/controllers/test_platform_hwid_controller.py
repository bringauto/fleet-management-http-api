...
class Test_Creating_Platform_HW_Id(unittest.TestCase):
    ...
    def test_creating_platform_hw_id_with_already_taken_id_returns_code_400(self):
        ...
        with self.app.test_client() as c:
            response = c.post('/v2/management/platformhwid', json=platform_hw_id_1)
            self.assertEqual(response.status_code, 200)
            response = c.post('/v2/management/platformhwid', json=platform_hw_id_2)
    ...
    def test_creating_platform_hw_id_with_missing_id_or_name_returns_code_400(self):
        with self.app.test_client() as c:
            response = c.post('/v2/management/platformhwid', json={"name": "test_platform"})
            self.assertEqual(response.status_code, 400)
            response = c.post('/v2/management/platformhwid', json={"id": 1})
            self.assertEqual(response.status_code, 400)
...
class Test_Adding_Platform_HW_Id_Using_Example_From_Spec(unittest.TestCase):
    ...
    def test_adding_state_using_example_from_spec(self):
        ...
        self.app = _app.get_test_app().app
        with self.app.test_client() as c:
            example = c.get('/v2/management/openapi.json').json["components"]["schemas"]["PlatformHwId"]["example"]
            response = c.post('/v2/management/platformhwid', json=example)
            self.assertEqual(response.status_code, 200)
...
class Test_Getting_Single_Platform_HW_Id(unittest.TestCase):
    ...
    def test_getting_single_existing_platform_hw_id(self):
        ...
        with self.app.test_client() as c:
            c.post('/v2/management/platformhwid', json=platform_hw_id_1)
            c.post('/v2/management/platformhwid', json=platform_hw_id_2)

            response = c.get('/v2/management/platformhwid/15')
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.json["id"], 15)

            response = c.get('/v2/management/platformhwid/24')
            self.assertEqual(response.status_code, 200)
    ...
    def test_retrieving_nonexistent_platform_hw_id_yields_code_404(self):
        ...
        with self.app.test_client() as c:
            response = c.get(f'/v2/management/platformhwid/{nonexistent_platform_id}')
