...
class Test_Creating_Car_DB_Model(unittest.TestCase):
    ...
    def test_car_with_all_attributes_specified_converted_to_db_model_and_back_is_unchanged(self):
        car_in = Car(name='test_car', platform_hw_id=1, car_admin_phone=MobilePhone(phone='1234567890'), default_route_id=1)
        car_out = _obj_to_db.car_from_db_model(_obj_to_db.car_to_db_model(car_in))
        self.assertEqual(car_out, car_in)
...
class Test_Creating_Car_State_DB_Model(unittest.TestCase):
    ...
    def test_creating_car_state_db_model_from_car_state_object_preserves_attribute_values(self):
        car_state = CarState(status="idle", car_id=1, speed=7, fuel=8, position=GNSSPosition(latitude=48.8606111, longitude=2.337644, altitude=50))
    ...
    def test_car_state_with_all_attributes_specified_converted_to_db_model_and_back_is_unchanged(self):
        state_in = CarState(status="idle", car_id=1, speed=7, fuel=8, position=GNSSPosition(latitude=48.8606111, longitude=2.337644, altitude=50))
        state_out = _obj_to_db.car_state_from_db_model(_obj_to_db.car_state_to_db_model(state_in))
    ...
    def test_car_state_db_model_has_timestamp_attribute_corresponding_to_time_of_the_instances_creation(self, mock_timestamp_in_ms: Mock):
        ...
        order_state = CarState(status="idle", fuel=50, car_id=2, speed=0, position=GNSSPosition(latitude=48.8606111, longitude=2.337644, altitude=50))
...
class Test_Creating_OrderStateDBModel(unittest.TestCase):
    ...
    def test_order_state_converted_to_db_model_and_back_preserves_its_attributes(self):
        ...
        order_state_out = _obj_to_db.order_state_from_db_model(_obj_to_db.order_state_to_db_model(order_state_in))
