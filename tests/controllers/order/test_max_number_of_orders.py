...
class Test_Number_Of_Active_Orders(unittest.TestCase):
    ...
    def test_the_number_is_unchanged_when_restarting_the_application(self):
        ...
        with self.app.app.test_client() as c:
            c.post("/v2/management/order", json=order)
            c.post("/v2/management/order", json=order)
            self.assertEqual(_app.n_of_active_orders(car_id=1), 2)
        self.app = _app.get_test_app()
        clear_active_orders()
        with self.app.app.test_client() as c:
            self.assertEqual(_app.n_of_active_orders(car_id=1), 2)
...
class Test_Number_Of_Inactive_Orders_Lower_Than_Maximum(unittest.TestCase):
    ...
    def test_is_increased_or_when_order_receives_done_status(self):
        ...
        with self.app.app.test_client() as c:
            c.post("/v2/management/order", json=order)
            self.assertEqual(_app.n_of_inactive_orders(1), 0)
            c.post("/v2/management/orderstate", json=done_state)
            self.assertEqual(_app.n_of_inactive_orders(1), 1)
    ...
    def test_the_number_is_unchanged_when_restarting_the_application(self):
        ...
        with self.app.app.test_client() as c:
            ...
            self.assertEqual(_app.n_of_inactive_orders(car_id=1), 2)
        self.app = _app.get_test_app()
        clear_active_orders()
        with self.app.app.test_client() as c:
            self.assertEqual(_app.n_of_inactive_orders(car_id=1), 2)
...
class Test_Automatic_Removal_Of_Inactive_Order(unittest.TestCase):
    ...
    def test_when_new_order_is_marked_as_done_and_maximum_number_of_inactive_orders_is_already_reached(self):
        _app.set_max_n_of_active_orders(None)
        _app.set_max_n_of_inactive_orders(2)
        order_1 = Order(user_id=1, target_stop_id=1, stop_route_id=1, car_id=1)
        order_2 = Order(user_id=1, target_stop_id=1, stop_route_id=1, car_id=1)
        order_3 = Order(user_id=1, target_stop_id=1, stop_route_id=1, car_id=1)
        with self.app.app.test_client() as c:
            ...
            c.post("/v2/management/orderstate", json=OrderState(status=OrderStatus.DONE, order_id=1))
            c.post("/v2/management/orderstate", json=OrderState(status=OrderStatus.DONE, order_id=2))
            self.assertEqual(_app.n_of_inactive_orders(1), 2)
