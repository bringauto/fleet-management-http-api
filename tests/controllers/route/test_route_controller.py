...
class Test_Retrieving_Route_Stops(unittest.TestCase):
    ...
    def setUp(self) -> None:
        set_connection_source_test()
        self.app = _app.get_test_app()
        stop_1 = Stop(name="test_stop_1", position=GNSSPosition(latitude=1, longitude=1, altitude=1))
        stop_2 = Stop(name="test_stop_2", position=GNSSPosition(latitude=2, longitude=2, altitude=2))
        stop_3 = Stop(name="test_stop_3", position=GNSSPosition(latitude=3, longitude=3, altitude=3))
