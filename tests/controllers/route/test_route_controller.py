...
class Test_Retrieving_Route_Stops(unittest.TestCase):
    ...
    def setUp(self) -> None:
        set_connection_source_test()
        self.app = _app.get_test_app()
        stop_1 = Stop(id=1, name="test_stop_1", position=GNSSPosition(latitude=1, longitude=1, altitude=1))
        stop_2 = Stop(id=2, name="test_stop_2", position=GNSSPosition(latitude=2, longitude=2, altitude=2))
        stop_3 = Stop(id=3, name="test_stop_3", position=GNSSPosition(latitude=3, longitude=3, altitude=3))

        self.route_1 = Route(id=78, name="test_route_1", stop_ids=[1, 2])
        self.route_2 = Route(id=142, name="test_route_2", stop_ids=[2, 3])
        with self.app.app.test_client() as c:
