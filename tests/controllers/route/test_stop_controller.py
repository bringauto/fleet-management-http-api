...
class Test_Creating_Stop(unittest.TestCase):
    ...
    def test_creating_stop_with_identical_name_yields_code_400(self):
        position = _models.GNSSPosition(latitude=49,longitude=16,altitude=50)
        stop_1 = _models.Stop(name="stop_X", position=position, notification_phone=_models.MobilePhone(phone="123456789"))
        stop_2 = _models.Stop(name="stop_X", position=position, notification_phone=_models.MobilePhone(phone="123456789"))
        with self.app.test_client() as c:
            response = c.post('/v2/management/stop', json=stop_1)
            self.assertEqual(response.status_code, 200)
            response = c.post('/v2/management/stop', json=stop_2)
...
class Test_Retrieving_All_Stops(unittest.TestCase):
    ...
    def test_retrieving_sent_stops(self):
        position = _models.GNSSPosition(latitude=49,longitude=16,altitude=50)
        stop_1 = _models.Stop(name="stop_X", position=position, notification_phone=_models.MobilePhone(phone="123456789"))
        stop_2 = _models.Stop(name="stop_Y", position=position, notification_phone=_models.MobilePhone(phone="123456789"))
        with self.app.test_client() as c:
...
class Test_Deleting_Stop(unittest.TestCase):
    ...
    def test_deleting_single_existing_stop(self):
        position = _models.GNSSPosition(latitude=49,longitude=16,altitude=50)
        stop = _models.Stop(name="stop_X", position=position, notification_phone=_models.MobilePhone(phone="123456789"))
        with self.app.test_client() as c:
...
class Test_Updating_Stop(unittest.TestCase):
    ...
    def test_updating_nonexisting_stop_yields_code_404(self):
        position = _models.GNSSPosition(latitude=49,longitude=16,altitude=50)
        stop = _models.Stop(id=116516556165, name="stop_X", position=position, notification_phone=_models.MobilePhone(phone="123456789"))
        with self.app.test_client() as c:
            response = c.put('/v2/management/stop', json=stop)
            self.assertEqual(response.status_code, 404)
...
class Test_Stop_Cannot_Be_Deleted_If_Assigned_To_Order(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.car = _models.Car(id=1, platform_hw_id=2, name="test_car", car_admin_phone=_models.MobilePhone(phone="123456789"))
