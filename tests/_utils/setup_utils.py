...
def create_platform_hws(tenant: str, app: _TestApp, count: int = 1) -> None:
...
def create_stops(tenant: str, app: _TestApp, count: int = 1) -> None:
    with app.app.test_client(tenant) as c:
        for i in range(count):
            stop = _models.Stop(
...
def create_route(tenant: str, app: _TestApp, stop_ids: tuple[int, ...]) -> None:
    with app.app.test_client(tenant) as c:
