...
def add_car_state(car_state) -> ConnexionResponse:
    if not connexion.request.is_json:
        ...
        car_db_model = db_access.get(db_models.CarDBModel, criteria={'id': lambda x: x==car_state.car_id})
        if len(car_db_model) == 0:
            code, msg = 404, f"Car with id='{car_state.car_id}' was not found."
            log_error(msg)
        else:
            response = db_access.add(db_models.CarStateDBModel, state_db_model)
...
def get_all_car_states() -> ConnexionResponse:
    car_state_db_models = db_access.get(db_models.CarStateDBModel)
...
def get_car_states(car_id: int, all_available: bool = False) -> ConnexionResponse:
    if not _car_exists(car_id):
        ...
        car_state_db_models = db_access.get(db_models.CarStateDBModel, criteria={'car_id': lambda x: x==car_id})
