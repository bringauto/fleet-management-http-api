...
import fleet_management_api.database as db_access
...
def create_route(route: Route) -> ConnexionResponse:
    if not connexion.request.is_json:
        ...
        response = db_access.add(RouteDBModel, route_db_model)
...
def delete_route(route_id: int) -> ConnexionResponse:
    response = db_access.delete(RouteDBModel, id_name="id", id_value=route_id)
    if response.status_code == 200:
...
def get_route(route_id: int) -> Route:
    route_db_models = db_access.get(RouteDBModel, criteria={"id": lambda x: x==route_id})
...
def get_routes() -> List[Route]:
    route_db_models = db_access.get(RouteDBModel)
...
def update_route(route: Route) -> ConnexionResponse:
    if connexion.request.is_json:
        route = Route.from_dict(connexion.request.get_json())
        route_db_model = obj_to_db.route_to_db_model(route)
        response = db_access.update(updated_obj=route_db_model)
