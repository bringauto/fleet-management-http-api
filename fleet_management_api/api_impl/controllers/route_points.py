...
def redefine_route_points() -> _Response:
    ...
    if not _connexion.request.is_json:
        ...
        existing_route_points = _db_access.get(
            _db_models.RoutePointsDBModel, criteria={"route_id": lambda x: x == rp.route_id}
        )
        if len(existing_route_points) == 0:
            response = _db_access.add(
                rp_db_model,
                check_objs=[_db_access.check_obj(_db_models.RouteDBModel, rp.route_id)]
            )
