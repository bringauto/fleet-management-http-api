...
from fleet_management_api.api_impl.controllers.car_state import (
    add_car_state_from_argument as _add_car_state_from_argument
)
...
def get_car(car_id: int) -> _Response:
    ...
    if len(cars) == 0:
        ...
        car = _obj_to_db.car_from_db_model(cars[0])
        car.last_state = _obj_to_db.car_state_from_db_model(last_state)
...
def get_cars() -> _Response:  # noqa: E501
    ...
    cars = [_obj_to_db.car_from_db_model(c) for c in db_cars]
    if len(cars) == 0:
        ...
        for car in cars:
            ...
            car.last_state = _obj_to_db.car_state_from_db_model(last_state[0])
...
def _create_default_car_state(car_id: int) -> _Response:
