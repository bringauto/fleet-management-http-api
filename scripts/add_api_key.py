...
def _add_key_if_key_name_not_already_in_db(
    connection_source: Engine, api_key_name: str
) -> int:
...
if __name__ == "__main__":
    ...
    if config.database.test.strip() != "":
        source = _connection.get_connection_source_test(
            db_file_path=config.database.test
        )
